...
def main():
    ...
    input_name = 'H2C_S_TFLA_refined_new'
    ...
    gamma_1H = 267.522e6  # [rad s^-1 T^-1]
    h_bar = 1.05457266e-34 # [J * s]
    I = 1/2  # spin of resonant nuclei
    mu_0 = 1.256637e-6 # permeability of the vacuum [N A^-2]
    
    try: 
        second_moment = compute_homo_M2(mu_0, gamma_1H, h_bar, I, r_jk)
        print(f"The computed van Vleck Homonuclear 1H-1H second moment is: {second_moment}")
        
    except ValueError as error:
        print(f"Error: {error}")

    store_M2 = os.path.join(os.getcwd(), 'van_vleck_M2/M2_H2C_S_TFLA_values.txt')
